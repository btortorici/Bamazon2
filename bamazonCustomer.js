...
function afterConnection() {
  connection.query("SELECT id, product, price FROM products", function (error, results) {
    if (error) throw error;
    ...
    .then(answers => {
      connection.query(
        ...
        function (error, results) {
          ...
          if (results[0].quantity >= answers.stock) {
            let newAmountOfItems = results[0].quantity - answers.stock;

            connection.query(
              "UPDATE products SET ? WHERE ?",
              [
                {
                  quantity: newAmountOfItems
                },
                {
                  id: answers.nameId
                }
